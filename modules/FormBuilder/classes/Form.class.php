...
/* ]]> */
	...
	$smarty->assign('submit','<input class="cms_submit fbsubmit_next" name="'.$id.'fbrp_submit" id="'.$id.'fbrp_submit" value="'.$this->GetAttr('next_button_text').'" type="submit" '.$js.' />');  
      ...
      if ($this->GetAttr('use_captcha','0')== '1' && $captcha != null)
         ...
         if($cptcneedsinput) $smarty->assign('input_captcha',$mod->CreateInputText($id, 'fbrp_captcha_phrase',''));
...
//debug_display($elements);
  ...
  */
  ...
  function Store( $storeDeep = false )
    ...
    # TODO -JoMorg: change this to exception
    if(!$res)
    {
  ...
  function AddEditForm($id, $returnid, $tab, $message='')
  {
    ...
    if($this->Id > 0)
    {
      ...
      foreach ($this->Fields as $thisField)
      {
        ...
        if (!$thisField->DisplayInForm() || $thisField->IsNonRequirableField())
        {
          ...
          {
            $oneset->disposition = '<img src="'.$mod->GetModuleURLPath().'/images/stop.gif" width="20" height="20" alt="'.$no_avail.'" title="'.$no_avail.'" />';
          }
          else
          {
		        $oneset->disposition = '<img src="'.$mod->GetModuleURLPath().'/images/stop.gif" width="16" height="16" alt="'.$no_avail.'" title="'.$no_avail.'" />';
		      }
	      }
    ...
    if( !formbuilder_utils::is_CMS2_3() )
    {
      ...
      if($udt_handler_pref == 'core_sps') $udt_handler_pref = 'core_udts';
    ...
    else
    {
      ...
      if($udt_handler_pref == 'core_udts') $udt_handler_pref = 'core_sps';
  ...
  function AddEditField($id, &$aefield, $dispose_only, $returnid, $message='')
  {
    ...
    $smarty->assign('fb_hidden', $mod->CreateInputHidden($id, 'form_id', $this->Id) . $mod->CreateInputHidden($id, 'field_id', $aefield->GetId()) . $mod->CreateInputHidden($id, 'fbrp_order_by', $aefield->GetOrder()).
